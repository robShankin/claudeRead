...
A tool that converts Codex `.jsonl` session files into a human-readable (and agent-parseable) format. Target users are engineers debugging or reviewing their Codex workflows — they should not need to understand Anthropic's internal JSONL schema to use this tool.
...
## JSONL Schema (Codex session files)
...
| `assistant` | A Codex turn. `message.content` is an array of `text`, `tool_use`, or `thinking` blocks |
...
JSONL files live at `~/.Codex/projects/<encoded-path>/<session-id>.jsonl`.
